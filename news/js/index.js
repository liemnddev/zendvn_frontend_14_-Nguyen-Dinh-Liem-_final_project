$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          //let linkDetail = "detail.html?id=" + data[i].id;
          content += `
            <article class="item post col-md-6">${renderPost(data[i])}</article>
            `;
        }
  ...
  function renderPopularPost() {
    $.ajax({
      type: "GET",
      url: "http://apiforlearning.zendvn.com/api/articles/most-read",
      data: "data",
      dataType: "json",
      success: function (data) {
        let content = "";
        ...
        for (let i = 0; i < limit; i++) {
          content += `
                ...
                <figure class="rounded"><a href="./blog-post.html"><img
                            src="${data[i].thumb}" alt="" /></a></figure>
                <div class="post-content">
                    <h6 class="mb-2"> <a class="link-dark" href="./blog-post.html">${data[i].title}</a> </h6>
                    <ul class="post-meta">
                        <li class="post-date"><i class="uil uil-calendar-alt"></i><span>${data[i].publish_date}</span></li>
  ...
  function renderListCategory() {
    $.ajax({
      type: "GET",
      url: "http://apiforlearning.zendvn.com/api/categories_news/list-with-count-articles",
      data: "data",
      dataType: "json",
      success: function (data) {
        let content = "";
        ...
        for (let i = 0; i < data.length; i++) {
          console.log(i);
          content += `
                  <li><a href="#">${data[i].name} (${data[i].articles_count})</a></li>
            `;
        }
  ...
  function renderSlide() {
    $.ajax({
      type: "GET",
      url: "http://apiforlearning.zendvn.com/api/articles/top-articles",
      data: {
        offset: 3,
        limit: 4,
      },
      ...
      success: function (data) {
        ...
        let allComments = JSON.parse(localStorage.getItem("comment")) || [];
        let comments = allComments.filter(function (element) {
          return element.artiles_id == data.id;
        });
        // console.log(comments);
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          <div class="swiper-slide"> 
            <figure class="overlay caption caption-overlay rounded mb-0"><a href="${linkDetail}"> <img src="${data[i].thumb}" alt="" /></a>
              <figcaption>
                  <span class="badge badge-lg bg-white text-uppercase mb-3">${data[i].category.name}</span>
                  <h2 class="post-title h3 mt-1 mb-3"><a href="./blog-post.html">${data[i].title}</a></h2>
